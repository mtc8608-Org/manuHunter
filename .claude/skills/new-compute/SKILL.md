...
1. **Python endpoint** — `python/api/domains/<domain>/routes.py` (clone `domains/compute/routes.py` as the pattern): `router = APIRouter(prefix="/<domain>")`, pydantic request models, numbered module docstring stating what it computes and that Node persists. New domain → `app.include_router(...)` in `api/main.py`.
2. **Node caller** — a REST route in `nodejs/routes/<domain>/` (register in `backend.js`; see the `new-api` skill for route conventions) that:
   ...
   - persists or streams the result itself, translating Python error bodies into useful client errors (`compileErrorPayload` pattern in `routes/cv/compile.js`).
