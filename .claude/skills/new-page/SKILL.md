...
- `.claude/rules/page-template.md` — the four SplitPageLayout left-column rules
...
2. **Clone the closest existing page** in `pwa/src/pages/` as the starting point (e.g. a list+detail page → `GeneratedCvs.tsx`; list + tabbed detail → `Applications.tsx`). Do not write a page from scratch.
...
5. **Register the page:**
   ...
   - Constants (`AREA_NAV`, panel configs, form names) in `pwa/src/constants.ts`, marked `// [MY DOMAIN]`.
