...
- **A whole new area** → one `AREA_NAV.<AREA>` list *and* one `NAV_AREAS` entry (`constants.ts`), plus its route in `App.tsx`. `NAV_AREAS` is the single nav source: the drawer (`Menu`), the top-bar sections (`NAV_SECTIONS`, derived) and the in-page rail all read it, so **never hand-edit `Menu.tsx` or `NAV_SECTIONS` to add navigation**. Icons are named as strings and resolved through `components/shell/icons.ts` — a new icon means one import plus one line there.
