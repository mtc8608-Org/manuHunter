...
A form is a `form` root node with field children in the `components` table (`survey_components` for surveys, `cv_components` editor forms still live in `components`):

- Field key = `options.label` — **name it exactly after the DB column / mutation arg it feeds** (`{"label": "job_url"}`), so `FormRenderer`'s output object maps straight onto the GraphQL mutation's args with no translation layer (jobs pattern).
...
- Forms are fetched **by name** (`componentByName` via `Api.ts`), with the name in a `constants.ts` group (`FORM_ID`, `APP_FORM`, `CV_FORM`, `EDITOR_ID`, …) whose comment names the init script that seeds it as source of truth.
